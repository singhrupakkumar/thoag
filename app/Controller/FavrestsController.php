 ...
 */
	public function add() {
		if ($this->request->is('post')) {
                      ...
                      $cnt = $this->Favrest->find('count', array('conditions' =>
                      array('AND' => array('Favrest.user_id' => $this->request->data['Favrest']['user_id'] ,
                          'Favrest.res_id' =>$this->request->data['Favrest']['res_id']))));
            if ($cnt == 0) {  
           ...
           $this->Session->setFlash('This Restaurant already add in Your favrest .', 'flash_success');
            return $this->redirect('http://' .$_POST['server']);
        }
	}
      }
