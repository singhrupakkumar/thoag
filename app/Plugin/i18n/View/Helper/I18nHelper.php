 ...
 */
	public function flagSwitcher($options = array()) {
		...
		if (!empty($langs)) {
			...
			$out .= '<ul class="' . $options['class'] . '"' . $id . '>  <li><a href="#" data-toggle="modal" data-target="#catereregisterModal">Join as Caterer</a></li> ';
			
                        $langlabel = array('English (E)','العربية (أر)'); 
                        $i =0;
				...
				$out .= '<li class="' . $class . '">' .
					//$this->Html->link($this->flagImage($lang, $options), $url, array('escape' => false)) .
                                        $this->Html->link($langlabel[$i], $url, array('escape' => false)) .
				'</li>';
