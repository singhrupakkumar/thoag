...
//                                      
                 ...
                 $this->Session->setFlash('Unable to add this product to your shopping cart.', 'flash_error');  
                   return $this->redirect('http://rajdeep.crystalbiltech.com' . $this->request->data['server']);  
						     ...
						     $this->Session->setFlash($cart_product['Product']['name'].' is Out of stock!', 'flash_error');  
                            return $this->redirect('http://rajdeep.crystalbiltech.com' . $this->request->data['server']);    
                        }
                }else{
                   $added = $this->Cart->add($id,$uid, $quantity, $productmodId,$parent_id,$order_type,$notes);
            ...
            if ($this->Order->validates()) {
             ...
             $session_id =   $this->Session->id();     
              ...
              ///////////////////////////////////
				...
				if(empty($user['User']['phone'] && $user['User']['address'])){
					$this->Session->setFlash('Please enter Address/Contact Details!', 'flash_error');  
                $this->redirect(array('action' => 'address')); 
					
			    }elseif(empty($user['User']['phone'])){
				$this->Session->setFlash('Please enter Contact No. where you want to get update for Order!', 'flash_error');  
                $this->redirect(array('action' => 'address')); 	
				}elseif(empty($user['User']['address'])){
				$this->Session->setFlash('Please enter Address!', 'flash_error');  
                $this->redirect(array('action' => 'address')); 	
...
exit;     
        ...
        foreach ($data as $d) {
            ...
            if($d['Cart']['quantity']>1){
				...
				$response['msg'] = 'Min. Quantity allowed for this Product: '.$min;
			}
        }
   $response['cartdata'] = $this->webdisplaycartsecond();  
echo 	json_encode($response);
...
//                    print_r($order); exit;
                    ...
                    if($error == 1){
                        $response['error'] = "1";
                        $response['msg'] = $product_name ." is Out of Stock!";
                    }else{
                    $this->loadModel('Order');
                    $save = $this->Order->saveAll($order, array('validate' => 'first'));
