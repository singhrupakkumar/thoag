...
//            
            ...
            if($this->Notification->save($info)){
                ...
                //App::uses('CakeEmail', 'Network/Email');
               /* $email = new CakeEmail();
                $email->from('work@rajdeep.crystalbiltech.com')
...
//         exit;
        ...
        if ($this->Session->check('AOrder')) {
            ...
            $all = array(
                ...
                'filter' => '',
                'conditions' => array(
                    'Order.restaurant_id' => $res_first_data['Restaurant']['id'],'Order.order_status !=' => 0));
        } else {
          $all = array(
                'name' => '',
                'filter' => '',
...
////////////////////////////////////////////////////////////
    ...
    public function orderhistry($id= null){
		...
		$dispute = $this->Dispute->find('first',array('conditions'=>array('Dispute.order_id'=>$id,'Dispute.user_id'=>$uid))); 
		
        $this->Order->recursive= 1;
        $data = $this->Order->find('first',array('conditions'=>array('Order.id'=>$id,'Order.uid'=>$uid))); 
